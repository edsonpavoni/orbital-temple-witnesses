# firmware-v1.0 — Knowledge

Production firmware, v1.0, 2026-05-04. Forked from
`experiments/16_witness_choreography`. The boot ritual, calibration,
Wi-Fi/NTP, schedule fetch, captive portal, NVS layout, power gate, and motor
I²C are **identical** to 16 (and 14 before it). For foundational material see
[`../experiments/14_witness_provision/KNOWLEDGE.md`](../experiments/14_witness_provision/KNOWLEDGE.md).

This file documents what's new and different in v1.0 vs 16.

---

## 1. What 17 does

After the boot ritual lands the pointer at visual zero, control hands to
the Choreographer, which runs a recurring ~110-second cycle forever:

```
A. ENTER_SAT  ~8 s   — Tracker eased cosine ramp to live sat az
B. HOLD_SAT   30 s   — frozen at the captured az; tracker disabled
C. SPIN       ~42 s  — SpeedBrakeSpin: speed-mode cruise (8 RPM, loose PID)
                        + position-mode brake at 1800°. No drop before spin.
D. SPIN_HOLD  30 s   — position-mode hold at brake landing (Choreographer owns)
E. → re-acquire live sat position, loop to A
```

Total ~110 s. Forked 2026-05-04; pending hardware validation.
...
```
firmware-v1.0/
├── README.md                  # operational summary + flash instructions (updated for v1.0)
├── KNOWLEDGE.md               # this file
├── platformio.ini
├── partitions_witness.csv
├── src/
│   ├── SpeedBrakeSpin.{h,cpp}         NEW (v1.0): 09-style speed+brake 1800° spin
│   ├── Choreographer.{h,cpp}          NEW in 16, UPDATED in v1.0 (SPIN_HOLD, sync fix)
│   ├── MotorIO.h                      extended (speed-mode primitives)
│   ├── MotorState.h                   extended (ST_CHOREO_SPIN)
│   ├── Recipes.h                      extended (SB_* + BRAKE_* constants)
│   ├── Tracker.h                      extended (isEntering())
│   ├── main.cpp                       extended (SpeedBrakeSpin instantiation, banner)
│   └── (everything else identical to 14_witness_provision)
└── tools/                             unchanged from 14 (lab.py, etc.)
```

Note: `ChoreoSpin.{h,cpp}` existed in `experiments/16_witness_choreography/`
and was ported forward as a compile-clean reference but removed from v1.0 in
the final cleanup. It is preserved in `experiments/16_witness_choreography/src/`.
