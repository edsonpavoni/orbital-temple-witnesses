/*
 * ============================================================================
 * FIRST WITNESS — smooth speed-brake cycle (17_witness_choreography_smooth)
 * ============================================================================
 *
 * Forked from 16_witness_choreography on 2026-05-04. All boot/calibration/
 * network plumbing identical. Choreography loop updated:
 *
 *   Boot ritual (UNCHANGED, ends at visual zero):
 *     1. Power gate (Vin >= 14 V).
 *     2. Wi-Fi connect + NTP sync.
 *     3. Schedule fetch (or use cache).
 *     4. 3-second pre-delay.
 *     5. Sensorless homing (Calibrator).
 *     6. 1080° position-mode spin — lands precisely at visual zero.
 *
 *   Choreography loop (runs forever after boot):
 *     A. ENTER_SAT:  8 s eased cosine to live satellite az.
 *     B. HOLD_SAT:   freeze at that az for 30 s.
 *     C. SPIN:       speed-mode cruise (gravity-alive, loose PID) ramps up
 *                    to 8 RPM over 2 s, holds for ~38 s, fires position-mode
 *                    brake at 1797° and locks at exactly 1800°. No drop before
 *                    the spin — motor stays output-ON throughout. ~42 s total.
 *     D. SPIN_HOLD:  30 s position-mode hold at the brake landing angle.
 *     E. Loop to (A) with live satellite re-acquisition (sky has moved).
 *
 *   Total cycle: ~8 + 30 + 42 + 30 = ~110 s.
 *
 * Key change from 16: replaces ChoreoSpin (07-style ramp-down, non-deterministic
 * landing) with SpeedBrakeSpin (09-style speed+brake, 1800 deg, precise landing).
 * Removes the pre-spin drop (RELEASE_1) and adds a post-spin hold (SPIN_HOLD).
 *
 * If the schedule is unavailable, ENTER_SAT falls through and the pointer
 * holds at its current angle through the HOLD window — the artwork keeps
 * cycling even fully offline.
 * ============================================================================
 */
...
static const bool BOOT_CHOREO_ENABLED = true;

// PRODUCTION_MODE controls the schedule fetch cadence.
//   false (dev/current): forcefetch hourly — bypasses fetchDue 24 h policy
//                        so cache staleness can never accumulate. Costs one
//                        HTTPS request per hour; trivial.
//   true  (production):  use the fetchDue policy (currently 24 h, intended
//                        to be tightened to 6 h in ScheduleStore::fetchDue
//                        when we ship).
// Background: 2026-05-04, OW1/12 was running with a stale cache and the UI
// showed 17° ahead of the sculpture. forcefetch closed the gap. Until the
// policy is tightened in code, periodic forcefetch is the workaround.
// TODO[production]: flip to true AND tighten fetchDue() in ScheduleStore.
static const bool PRODUCTION_MODE = false;
...
SpeedBrakeSpin choreoSpin;
Choreographer  choreo(motor, tracker, choreoSpin, moveOp, schedule);
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  " SCULPTURE_NAME " — smooth speed-brake (17)");
  Serial.println("============================================");
  Serial.println("Boot: power gate -> wifi+ntp -> schedule fetch -> homing");
  Serial.println("      -> 1080 spin -> CHOREO loop:");
  Serial.println("      enter sat (8s) -> hold (30s) -> speed-brake spin");
  Serial.println("      1800 CW (~42s) -> hold at landing (30s) -> repeat.");
  Serial.println("Commands: release | hold | diag | home | spin |");
  Serial.println("          move <deg> | track | untrack | fetch |");
  Serial.println("          choreo | chstop |");
  ...
  Serial.print("BOOT_CHOREO:     "); Serial.println(BOOT_CHOREO_ENABLED ? "true" : "false");
  Serial.println("Firmware:        17_witness_choreography_smooth");
  Serial.println("Cycle:           enter-sat(8s) -> hold(30s) -> spin-1800CW(~42s) -> spin-hold(30s)");
  Serial.print("Schedule policy: ");
  Serial.println(PRODUCTION_MODE ? "fetchDue (24h cache)" : "hourly forcefetch (dev — UI/sculpture alignment)");
  Serial.println("============================================");
  Serial.println();
...
void loop() {
  ...
  // After the boot spin lands at visual zero, kick off the choreography
  // loop. One-shot: arms once at boot, fires the first time we see
  // ST_HOLDING after the boot 1080° has completed. The pendingSpin handler
  // above transitions ST_HOLDING → ST_SPIN_RAMP_UP within the same
  // iteration if the boot move-to-zero just finished, so this trigger only
  // fires once the boot spin has actually settled.
  static bool s_choreoArmed = BOOT_CHOREO_ENABLED;
  if (s_choreoArmed && motor.state == ST_HOLDING && !choreo.isActive()) {
    s_choreoArmed = false;
    choreo.start();
  }
  ...
  // Periodic schedule refresh. PRODUCTION_MODE (see above) selects between
  // the policy-respecting tryFetchScheduleIfDue (24 h cache window) and a
  // forcefetch (bypasses the policy). Until the fetchDue() policy is
  // tightened to ~6 h, the forcefetch path is what keeps UI/sculpture
  // aligned over a long run.
...
void processSerialCommand(const String& cmd) {
  ...
  if (low == "choreo") {
    ...
    if (choreo.isActive()) {
      Serial.println(">> Choreographer already active. Use `chstop` to stop.");
      return;
    }
    choreo.start();
    return;
...
void printReport() {
  const char* stateName =
      ...
      (motor.state == ST_CHOREO_SPIN)  ? "CHOREO_SPIN"  : "?";
  Serial.println();
  ...
  Serial.print("Choreographer:   ");
  Serial.println(choreo.isActive() ? choreo.phaseName() : "idle");
  Serial.println("==============");
}
