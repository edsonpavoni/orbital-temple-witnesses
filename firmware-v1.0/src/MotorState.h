...
enum State {
  ...
  ST_CHOREO_SPIN,        // 07-style speed-mode spin (ChoreoSpin-driven).
                         // Distinct from ST_SPIN_CRUISE so the position-mode
                         // SpinOperator (boot ritual) doesn't fire its tick
                         // logic during the choreography spin.
};
