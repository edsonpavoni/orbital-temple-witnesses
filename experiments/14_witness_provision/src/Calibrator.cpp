...
void Calibrator::onDiagComplete() {
  // Solve the sinusoid fit. NOTE: samples were accumulated in the Logger's
  // emit path (it's the canonical source of truth for the per-tick samples).
  // We reach into _fit which the Logger has been feeding via accumulate().
