...
bool ScheduleClient::fetch(ScheduleStore& store, float lat, float lon, uint32_t nowUTC) {
  ...
  // Pack into 4 B / sample (int16 deci-degrees). Halves the NVS footprint
  // vs the original float pair, so the full 1440 × 1-min schedule fits
  // alongside Wi-Fi creds with no decimation. Decimating to 5-min was
  // visibly wrong (~40° transient errors during fast az motion of an
  // overhead pass).
  ...
  for (JsonObject s : samples) {
    ...
    float az = s["az"] | 0.0f;
    float el = s["el"] | 0.0f;
    buf[i].az_deci = (int16_t)lroundf(az * 10.0f);
    buf[i].el_deci = (int16_t)lroundf(el * 10.0f);
    i++;
  }
  h.samples_count        = (uint16_t)i;
  h.samples_interval_sec = doc["samples_interval_sec"] | 60;
