...
- **Engage** at Vin ≥ 14.0 V (lowered 2026-05-01; see note below).
...
**2026-05-01 — engage threshold lowered 15.0 → 14.0 V.** The exhibition
PD charger arrived and negotiates the 15 V profile at a steady
**14.91 V** (well within the USB-PD ±5 % spec for that profile, range
14.25–15.75 V). The previous 15.0 V engage was 0.09 V above what the
charger actually delivers, so the firmware sat in the wait loop forever:

```
>> Waiting: Vin=14.91 V (need >= 15.0 V)
```

Lowering `ENGAGE_VIN_V` to 14.0 V cleared the gate and the sculpture
booted, homed, and tracked normally on the exhibition charger. Motor
torque is unchanged (still operating with PD-15 V profile, just naming
the threshold honestly).

**Hysteresis side-effect:** engage 14.0 / drop 13.5 leaves only 0.5 V
of hysteresis (was 1.5 V). This is fine for the exhibition charger
because Vin is rock-stable at 14.91 V. If we ever see flapping mid-show,
drop `DROP_VIN_V` to 13.0 V to restore a 1.0 V band.
...
ENGAGE_VIN_V         14.0   (boot ritual will not start below this; lowered 2026-05-01)
