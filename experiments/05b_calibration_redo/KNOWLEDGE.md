...
- **Power gate.** The XIAO ESP32-S3 boots fine on USB 5 V, but the
  ...
  torque. The firmware waits for `Vin >= 14 V` before engaging
  (the threshold is named conservatively — the PD-15 V profile is
  still requested; 14 V just gives headroom against measurement noise
  and PD-spec ±5 % tolerance, see STAGE1_KNOWLEDGE.md 2026-05-01 note).
...
POWER GATE (validated in 06; firmware checks Vin)
  min_vin_v          = 14.0    (engage threshold; lowered 2026-05-01 from 15.0
                                to clear ±5 % PD tolerance — see STAGE1 note)
```
