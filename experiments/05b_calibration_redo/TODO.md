...
- **Quieter sculpture.** Research ways to bring the motor and pointer
  acoustics down. Candidates: damping the BLDC's whine via PID retuning
  (lower Kp tail at micro-corrections), softer stop transitions, mass
  damping on the pointer carrier, isolation between motor housing and
  the wall plate. Goal: gallery-quiet at conversational distance.

- **Motion ritual.** Design a deliberate gestural ritual the sculpture
  performs at chosen moments — e.g., on first tracker lock after boot,
  or on a periodic cadence. The current 1080° boot spin is functional
  (proves the rotor is alive, settles the gravity homing); the ritual
  here would be expressive instead — a phrase of motion that reads as
  intentional. Decide trigger points, choreograph velocity envelope,
  decide whether it's deterministic or seeded by something (sat
  visibility, time of day, fetch-day boundary).

## Per-sculpture identity ("Orbital Witness N/12")

Each sculpture should carry its own name as a universal project
variable used everywhere it's surfaced:

- **SoftAP SSID during provisioning** — currently `FirstWitness-XXXX`.
  Should be the sculpture's own name, e.g. `Orbital Witness 1/12`.
- Captive-portal page title and heading.
- `report` serial output.
- Future: identifier the sculpture sends with its schedule fetches /
  telemetry, so the webservice can address each unit individually.

Implementation: add `SCULPTURE_NAME` to `secrets.h` (or a separate
`identity.h`) so each unit's firmware build carries its own name.
Replace hardcoded `"FirstWitness-"` and friends with this constant.

This unit: **Orbital Witness 1/12**.

**Resolved in 12 (offline behaviour):**
...
- [x] NVS persistence reliability.

**Resolved in 13 (this stage — IP geolocation):**
- [x] Auto-detect observer lat/lon via `ipapi.co` on first online boot.
- [x] Persist to NVS (namespace `geo`); subsequent boots load from NVS.
- [x] Hardcoded fallback in `secrets.h` if both NVS and IP geo fail.
- [x] `geo` / `regeo` serial commands for inspection + re-deployment.
