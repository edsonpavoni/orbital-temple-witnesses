/*
 * ============================================================================
 * FIRST WITNESS — calibrate + spin (final unified firmware)
 * ============================================================================
 *
 * On every power-up the sculpture autonomously:
 *   1. Discovers its absolute orientation via sensorless gravity-based homing
 *      (analyses motor-current sinusoid during a 2-direction sweep).
 *   2. Rotates to true visual vertical.
 *   3. Performs one 1080° (3 full revolutions) spin in position-mode
 *      rate-integrated control, braking at exactly visual zero.
 *   4. Holds at visual zero indefinitely.
 *
 * No hand-positioning ever required after the one-time mass-offset
 * calibration is saved to NVS.
 *
 * ============================================================================
 * BOOT RITUAL (≈ 110 s)
 * ============================================================================
 *
 *   t=0     Boot, init motor, load mass_offset_deg from NVS.
 *   t=2     "Kick in 3 s..." — user can step back.
 *   t=5     enterHoldHere — latch current encoder as user-frame zero.
 *   t=5.5   Switch to looser diagnostic PID so gravity lag is visible.
 *   t=5.5+  Diag CW: 2 full revolutions. Only the SECOND rev feeds the fit
 *           (rev 1 is the motor breaking static friction; samples biased).
 *   t≈42    Pause 1.5 s.
 *   t≈44    Diag CCW: 2 revolutions back to start. Same rev-2-only sampling.
 *   t≈80    End of CCW → solve 3×3 LS sinusoid fit on the accumulated
 *           (encoder, current) samples. Phase of fit = gravity-up angle.
 *           Apply mass_offset_deg → visual-up angle. Re-anchor zero so
 *           user-frame 0 = visual-up. Restore production PID. Move there.
 *   t≈82    SPIN1080: position-mode rate-integrated cruise. Velocity
 *           setpoint cosine-eased 0 → 8 → "freeze". Setpoint advances at
 *           48 deg/s, rotor's position PID tracks. Brake = freeze setpoint
 *           at start + 1080° while rotor still moving; stiff PID arrests it.
 *   t≈110   ST_HOLDING at visual zero. Done.
 *
 * ============================================================================
 * CONTROL RECIPES
 * ============================================================================
 *
 *   Production (move-to-target, spin cruise, brake): used during normal
 *   motion. Tight tracking, ±0.5° accuracy validated by 04_position_lab.
 *     kp=30,000,000  ki=1,000  kd=40,000,000  mc=1000 mA (1500 mA during brake)
 *     easing=EASE_IN_OUT, mt=33.33 ms per deg of move distance
 *
 *   Diagnostic (sweep during calibration only): looser loop so gravity-
 *   induced lag becomes legible in current draw.
 *     kp=1,500,000   ki=30     kd=40,000,000  mc=1000 mA
 *     easing=EASE_IN_OUT, mt=50 ms/deg (~5 RPM avg)
 *
 * ============================================================================
 * SERIAL COMMANDS (115200 8N1) — for ad-hoc tuning, not needed at runtime
 * ============================================================================
 *
 *   release          Disable motor output (free-coast for hand-positioning).
 *   hold             Latch current encoder as user 0, hold in position mode.
 *   diag             Run a 2-rev sweep without applying offset (research).
 *   home             Re-run calibration + move to visual up + auto-spin.
 *   spin             Trigger a 1080° spin (must be holding).
...
//
// Default 21.74 deg — measured 2026-04-30 with a fresh hand-positioned
// recalibration after the pointer was re-mounted (the prior 10.83 deg from
// the 04-29 calibration was ~11° off after re-mounting). Stored in NVS so
// it survives power cycles; this default only kicks in if NVS is wiped.
// Override via `setcal <deg>` and persist via `savecal`.

#define MASS_OFFSET_DEG_DEFAULT  21.74f
...
enum State {
  ...
  ST_SPIN_RAMP_UP,    // 1080° spin: eased 0 → 8 RPM setpoint advance
  ST_SPIN_CRUISE,     // 1080° spin: setpoint advances at 8 RPM
  ST_SPIN_BRAKE,      // 1080° spin: setpoint frozen at start+1080°, PID brakes rotor
  ST_SPIN_SETTLE,     // 1080° spin: hold then back to HOLDING
};
...
// =============================================================================
// SPIN-AND-BRAKE (08-style 1080° rotation, used after homing)
// =============================================================================
// Position-mode rate-integration: each 100 Hz tick we advance currentTargetDeg
// by currentSpeedRPM × 6 deg/s × 0.01 s. The motor's production position PID
// tracks that slowly-moving setpoint. At end of cruise we freeze the setpoint
// at exactly start + 1080°; PID arrests the rotor's residual velocity.
//
// Validated as 08_brake1080/. See that README for the design rationale.
const float    SPIN_TOTAL_DEG       = 1080.0f;
const float    SPIN_CRUISE_RPM      = 8.0f;
const uint32_t SPIN_RAMP_MS         = 2000;
const uint32_t SPIN_BRAKE_GRACE_MS  = 300;
const uint32_t SPIN_SETTLE_MS       = 2000;
const int32_t  SPIN_BRAKE_MAX_MA    = 1500;     // bumped during spin for brake authority

float    spinStartTargetDeg     = 0.0f;
float    spinTargetEndDeg       = 0.0f;
float    spinCurrentSpeedRPM    = 0.0f;
float    spinStartSpeedRPM      = 0.0f;
float    spinTargetSpeedRPM     = 0.0f;
bool     spinInTransition       = false;
uint32_t spinTransitionStartMS  = 0;
uint32_t spinPhaseStartMS       = 0;
bool     g_pendingSpinAfterHome = false;
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  FIRST WITNESS — CALIBRATE + 1080 SPIN");
  Serial.println("============================================");
...
void startSpin1080() {
  // Caller (the ST_MOVING-complete handler) has just left the motor in
  // production-PID position-mode at currentTargetDeg = 0 (= visual up).
  // Bump the position-mode current cap so the brake at the end has authority.
  writePosMaxCurrent(SPIN_BRAKE_MAX_MA);
  delay(10);

  spinStartTargetDeg     = currentTargetDeg;
  spinTargetEndDeg       = currentTargetDeg + SPIN_TOTAL_DEG;
  spinStartSpeedRPM      = 0.0f;
  spinCurrentSpeedRPM    = 0.0f;
  spinTargetSpeedRPM     = SPIN_CRUISE_RPM;
  spinTransitionStartMS  = millis();
  spinInTransition       = true;
  spinPhaseStartMS       = millis();
  state                  = ST_SPIN_RAMP_UP;

  Serial.print(">> SPIN1080 start=");
  Serial.print(spinStartTargetDeg, 2);
  Serial.print(" deg  target=");
  Serial.print(spinTargetEndDeg, 2);
  Serial.println(" deg");
}
...
void updateMotion() {
  ...
  // 1080° spin states: rate-integrate the velocity setpoint into
  // currentTargetDeg each tick; motor's position PID tracks. Brake by
  // freezing the setpoint at exact target.
  if (state == ST_SPIN_RAMP_UP || state == ST_SPIN_CRUISE ||
      state == ST_SPIN_BRAKE  || state == ST_SPIN_SETTLE) {
    if (spinInTransition) {
      uint32_t elapsed_t = millis() - spinTransitionStartMS;
      if (elapsed_t >= SPIN_RAMP_MS) {
        spinCurrentSpeedRPM = spinTargetSpeedRPM;
        spinInTransition    = false;
      } else {
        float prog  = (float)elapsed_t / (float)SPIN_RAMP_MS;
        float eased = easeInOut(prog);
        spinCurrentSpeedRPM = spinStartSpeedRPM +
                              (spinTargetSpeedRPM - spinStartSpeedRPM) * eased;
      }
    }

    // Advance setpoint during ramp_up + cruise (RPM × 6 deg/s × 0.01 s).
    if (state == ST_SPIN_RAMP_UP || state == ST_SPIN_CRUISE) {
      currentTargetDeg += spinCurrentSpeedRPM * 0.06f;
    }
    writeTargetDeg(currentTargetDeg);

    if (state == ST_SPIN_RAMP_UP && !spinInTransition) {
      state            = ST_SPIN_CRUISE;
      spinPhaseStartMS = millis();
      Serial.print(">> SPIN cruise  enc=");
      Serial.println(readEncoderDeg(), 2);
    }
    if (state == ST_SPIN_CRUISE) {
      if (currentTargetDeg - spinStartTargetDeg >= SPIN_TOTAL_DEG) {
        // BRAKE: snap setpoint to exact target, zero velocity.
        currentTargetDeg = spinTargetEndDeg;
        writeTargetDeg(currentTargetDeg);
        spinCurrentSpeedRPM = 0.0f;
        spinTargetSpeedRPM  = 0.0f;
        spinInTransition    = false;
        state               = ST_SPIN_BRAKE;
        spinPhaseStartMS    = millis();
    ...
    if (state == ST_SPIN_BRAKE) {
      if (millis() - spinPhaseStartMS >= SPIN_BRAKE_GRACE_MS) {
        state            = ST_SPIN_SETTLE;
        spinPhaseStartMS = millis();
      }
    }
    if (state == ST_SPIN_SETTLE) {
      if (millis() - spinPhaseStartMS >= SPIN_SETTLE_MS) {
        float finalEnc = readEncoderDeg();
        float err      = finalEnc - SPIN_TOTAL_DEG;
        ...
        // Restore production current cap for steady hold.
        writePosMaxCurrent(REC_MC_MA);
        state = ST_HOLDING;
      }
    }
  ...
  if (elapsed >= moveDurMS) {
    ...
    if (state == ST_DIAG_CW) {
      ...
      if (diagApplyHoming) {
        ...
        Serial.println(" ms)");
        startMoveTo(0.0f, mt);
        // Chain a 1080° spin after the move-to-zero completes.
        g_pendingSpinAfterHome = true;
      } else {
      ...
      // Production move complete. If a spin is queued (post-homing), launch it
      // now; otherwise just hold.
      if (g_pendingSpinAfterHome) {
        g_pendingSpinAfterHome = false;
        startSpin1080();
      } else {
        state = ST_HOLDING;
      }
...
void printReport() {
  const char* stateName =
      ...
      (state == ST_SPIN_RAMP_UP) ? "SPIN_RAMP_UP" :
      (state == ST_SPIN_CRUISE)  ? "SPIN_CRUISE"  :
      (state == ST_SPIN_BRAKE)   ? "SPIN_BRAKE"   :
      (state == ST_SPIN_SETTLE)  ? "SPIN_SETTLE"  : "?";
  Serial.println();
