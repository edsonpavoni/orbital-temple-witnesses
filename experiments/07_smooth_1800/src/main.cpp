/*
 * ============================================================================
 * ORBITAL TEMPLE - MOTION TEST (07_smooth_1800)
 * ============================================================================
 *
 * Forked from 07_smooth1080. Identical control logic. Change: spin target is
 * 1800 deg (5 full revolutions) instead of 1080 deg (3 revolutions).
 * 1800 deg mod 360 deg = 0, so the pointer returns to the same physical
 * angle it started at.
 *
 * New in this fork vs 07_smooth1080:
 *   - SPIN_TOTAL_DEG = 1800 (was 1080)
 *   - SPIN_CRUISE_END_DEG = 1752 (was 1032)
 *   - 'f' command: release motor (free-spin / no holding torque) so the
 *     pointer falls to its natural gravity rest position. Use before 'g'
 *     to define X = "wherever gravity leaves it". After pressing 'f', wait
 *     for the pointer to settle fully, then press 'g'.
 *
 * TEST PROCEDURE (release-then-go workflow):
 *   1. Press 'f'  -> motor releases, pointer falls to gravity rest (X).
 *   2. Wait a few seconds for the pointer to fully settle.
 *   3. Press 'g'  -> motor engages, captures current encoder as start,
 *                    runs 1800 deg smooth spin, locks back at X.
 *   4. Observe: pointer should return to within ~0.5 deg of where it was.
 *
 * SERIAL COMMANDS:
 * ----------------
 ...
 *
 * g           Smooth 1800 deg spin (5 revolutions), lock at start angle.
 *             Motor re-engages, captures current encoder position as X,
 *             runs the full profile, then position-PID corrects residual.
 *
 ...
 * EXAMPLES:
 * ---------
 * f           Release motor, let pointer settle to gravity rest
 * g           Spin 1800 deg, return to X
...
// =============================================================================
// 1800° SPIN STATE MACHINE
// =============================================================================
// 'g' command kicks off a smooth 5-revolution rotation that ends locked at the
// exact starting angle (1800° mod 360° = 0). Phases:
//   RAMP_UP   position-mode 0 -> 8 RPM (cosine eased, 2 s)
//   CRUISE    position-mode at 8 RPM (setpoint advances deterministically),
//             until the setpoint has traveled >= SPIN_CRUISE_END_DEG
//   RAMP_DOWN position-mode 8 -> 0 RPM (cosine eased, 2 s) -- integrates ~48°
//   LOCK      snap setpoint to exactly start + 1800°; position PID corrects
//             the small residual; hold 5 s, report error
//
// CRUISE_END_DEG derivation:
//   Cosine ramp 8 RPM -> 0 over 2 s at 100 Hz:
//   Each tick: inc = rpm * 360 / 60 / 100
//   Integral = sum over 200 ticks of 8 * 0.5 * (1 + cos(t*pi/200)) * 360/60/100
//            = 0.48 * 0.5 * (200 + 0)  [cos sums to ~0 over a half-period]
//            = 48 degrees
//   So CRUISE_END = 1800 - 48 = 1752 deg.
enum SpinPhase {
  SPIN_IDLE,
  SPIN_RAMP_UP,
  SPIN_CRUISE,
...
// Trigger ramp-down when the SETPOINT has covered (1800 - 48) = 1752 degrees.
// Cosine ramp 8->0 RPM over 2 s integrates to exactly ~48 deg (see derivation
// above). The final LOCK snaps to start + 1800 to absorb residual drift.
const float    SPIN_CRUISE_END_DEG  = 1752.0f;
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  ORBITAL TEMPLE - 07_smooth_1800");
  Serial.println("============================================");
  Serial.println();
  ...
  Serial.println("  g        Smooth 1800 deg spin (5 rev), lock at start angle.");
  Serial.println("           Start at X (gravity rest after 'f'), finish at X.");
...
void loop() {
  ...
  // Drive the 1800 deg spin state machine (no-op when idle).
  updateSpin();
...
// =============================================================================
// 'f' command: disable motor output so the pointer falls freely under gravity
// to its natural rest position. This defines X for the release-then-go test.
// After pressing 'f', wait for the pointer to fully settle, then press 'g'.
//
// If a spin is in progress, it is aborted cleanly before releasing.
...
void releaseMotor() {
  ...
  // Disable motor output. Pointer will fall to gravity rest (X).
  motorEnable(false);
  usePositionMode = false;  // Force speed-mode path on next 'g' so
                            // switchToPositionMode() runs and captures
                            // the settled encoder position as floatPosition.
...
void startSpin1800() {
  ...
  // tracks that setpoint. End point is deterministic (= start + 1800 deg).
  //
  // Always call switchToPositionMode() so it reads the current encoder and
  // stores it as floatPosition. This is critical after a release ('f'):
  // the encoder has moved to the gravity-rest angle, and we must anchor the
  // spin's start reference there, not at the old position-mode setpoint.
  switchToPositionMode();
  // switchToPositionMode briefly drops motor output, so the rotor may have
  // transient lag. Give the position PID a moment to pull to floatPosition
  // before anchoring the spin reference.
  delay(300);
...
void updateSpin() {
  ...
  switch (g_spinPhase) {
    ...
    case SPIN_CRUISE:
      ...
      // when the setpoint has covered (1800 - 48) = 1752 deg; the ramp-down
      // integrates the remaining ~48 deg, landing at 1800 deg total.
    ...
    case SPIN_RAMP_DOWN:
      if (!inTransition) {
        // Snap the setpoint exactly to start + 1800 deg to absorb floating-point
