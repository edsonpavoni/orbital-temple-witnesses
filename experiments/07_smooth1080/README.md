...
Forked from `03_motion_test` on **2026-04-29**. Goal: keep 03's smooth
gravity-affected rotation feel, but add a single command (`g`) that spins
**1080° (3 full revolutions) and locks the pointer at the exact starting
angle** (1080° mod 360° = 0).

## How `g` works

Four-phase state machine on top of 03's existing speed-mode rotation:

1. **RAMP_UP** — `startTransition(8.0)` ramps 0 → 8 RPM with cosine ease over
   `transitionTimeMS` (default 2000 ms).
2. **CRUISE** — speed mode at 8 RPM with the loose flash speed-PID (gravity
   ripple comes through). Terminates when traveled angle ≥ 1000° (cumulative
   encoder, not modular).
3. **RAMP_DOWN** — `startTransition(0.0)` ramps 8 → 0 RPM eased. Motor coasts
   another ~50–80° during this window.
4. **LOCK** — `switchToPositionMode()` then `motorSetPosition(start + 1080°)`.
   Motor's position PID (kp=30M, ki=1k, kd=40M, written into flash before each
   spin) corrects the residual so the pointer settles at exactly the starting
   orientation. Held 5 s, then final pos + error printed.

Speed PID stays at 03's loose recipe (P=1.5M, I=1k, D=40M) so cruise preserves
the pendulum-swing aesthetic. Stiff position control only kicks in for the
small residual at the end.

Use `x` (emergency stop) to abort mid-spin.

---

## Original 03 description
